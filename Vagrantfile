...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell, privileged: false, inline: <<-SHELL
    ...
    if ! sudo pgrep -x "ntpd" > /dev/null; then
        ...
        if ! sudo pgrep -x "ntpd" > /dev/null; then
          echo -e "NTP failed to start! It should be installed and running for SKP.\n Check /etc/ntp.conf for any issues and correct them first! \n Exiting."
          exit 1
        fi
    ...
    sudo -u postgres createdb -O $USER skp_testnet
    sudo service postgresql start

    git clone https://github.com/skelpycoin/skp-node.git

    cd /home/vagrant/skp-node
    #rm -fr node_modules
