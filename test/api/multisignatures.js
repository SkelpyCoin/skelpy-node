...
function sendSkp (account, i, done) {
	var randomSkp = node.randomSkp();
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomSkp / node.normalizer;
		}
		done();
	});
}

function sendSkpFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	sendSkp(multisigAccount, null, done);
});
...
describe('PUT /api/api/transactions/', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendSkpFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
