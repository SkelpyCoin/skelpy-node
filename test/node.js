...
node.skp = require('./skelpy-js');
node.skp.crypto.setNetworkVersion(network.pubKeyHash);
...
node.normalizer = 100000000; // Use this to convert SKP amount to normal value
...
// Random SKP amount
node.Skelpy = Math.floor(Math.random() * (100000 * 100000000)) + 1;
...
// Returns random SKP amount
node.randomSkp = function () {
...
node.randomAccount = function () {
	...
	account.publicKey = node.skp.crypto.getKeys(account.password, network).publicKey;
	account.address = node.skp.crypto.getAddress(account.publicKey, network.pubKeyHash);
