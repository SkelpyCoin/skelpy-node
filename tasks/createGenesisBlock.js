...
var totalpremine = 3300000000000000;
...
makeKeypair = function (seed) {
	return skelpyjs.crypto.getKeys(seed, networks[config.network]);
};
...
create = function (data) {
	...
	for (var i = 0; i < transactions.length; i++) {
		...
		var bytes = skelpyjs.crypto.getBytes(transaction);
...
var transactions = [];
var remainingfund = {};
skelpyjs.crypto.setNetworkVersion(networks[network_name].pubKeyHash);
...
premine.publicKey = skelpyjs.crypto.getKeys(premine.passphrase).publicKey;
premine.address = skelpyjs.crypto.getAddress(premine.publicKey, networks[config.network].pubKeyHash);

genesis.publicKey = skelpyjs.crypto.getKeys(genesis.passphrase).publicKey;
genesis.address = skelpyjs.crypto.getAddress(genesis.publicKey, networks[config.network].pubKeyHash);
...
for(var i=1; i<52; i++){
  var delegate = {
	...
	delegate.publicKey = skelpyjs.crypto.getKeys(delegate.passphrase).publicKey;
	delegate.address = skelpyjs.crypto.getAddress(delegate.publicKey, networks[config.network].pubKeyHash);

	// create delegate
  var createDelegateTx = skelpyjs.delegate.createDelegate(delegate.passphrase, delegate.username);
  createDelegateTx.fee = 0;
  createDelegateTx.timestamp = 0;
  createDelegateTx.senderId = delegate.address;
  createDelegateTx.signature = skelpyjs.crypto.sign(createDelegateTx,skelpyjs.crypto.getKeys(delegate.passphrase));
  createDelegateTx.id = skelpyjs.crypto.getId(createDelegateTx);
...
for(var i=0; i < genesisAccounts.length; i++){
  ...
  total += account.total;
	...
	//send skp to account
	var premineTx = skelpyjs.transaction.createTransaction(account.address, account.total, null, premine.passphrase);

	premineTx.fee = 0;
	premineTx.timestamp = 0;
	premineTx.senderId = premine.address;
	premineTx.signature = skelpyjs.crypto.sign(premineTx,skelpyjs.crypto.getKeys(premine.passphrase));
	premineTx.id = skelpyjs.crypto.getId(premineTx);
...
var preminefund = skelpyjs.transaction.createTransaction(genesis.address, remainingfund.total, null, premine.passphrase);

preminefund.fee = 0;
preminefund.timestamp = 0;
preminefund.senderId = premine.address;
preminefund.signature = skelpyjs.crypto.sign(preminefund,skelpyjs.crypto.getKeys(premine.passphrase));
preminefund.id = skelpyjs.crypto.getId(preminefund);
transactions.push(preminefund);

var genesisBlock = create({
  keypair: skelpyjs.crypto.getKeys(genesis.passphrase, networks[config.network]),
