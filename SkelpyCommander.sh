...
if [ "$(id -u)" = "0" ]; then
	...
	echo -e "\nUse $(green "bash SkelpyCommander.sh") as a REGULAR user instead"
	echo -e "Execute ONCE $(green "chmod +x Skelpycommandersh") followed by $(green "ENTER")"
	echo -e "and start it only by $(green "./SkelpyCommander.sh") as regular user after\n"
	exit 1
fi
...
if [ $(systemd-detect-virt -c) != "none" ]; then
	...
	echo "$(ired "                                                                                 ")"
	echo "$(ired "     Running SKELPY Node on a Container based virtual system is not recommended!    ")"
...
function proc_vars {
        ...
        Skelpydir=`locate -b skelpy-node`

        # Getting the parent of the install path
        parent=`dirname $Skelpydir 2>&1`
...
#	echo -e "$(green "Public Key:")\n$(yellow "$PUBKEY")\n"
	...
	echo -e "$(green "      Skelpy Balance      : ")$(yellow "$BALANCE")"
	echo
	echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
        if [ -e $Skelpydir/app.js ]; then
                echo -e "\n$(green "       ✔ Skelpy Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      Skelpy Node process is running with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $Skelpydir")\n"
                else
                        echo -e "\n$(red "       ✘ No Skelpy Node process is running")\n"
                fi
        else
                echo -e "\n$(red "       ✘ No Skelpy Node installation is found")\n"
        fi
	echo -e "\n$(yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")"
...
function stats {
	...
	if [ "$node" != "" ] && [ "$node" != "0" ]; then
		echo -e "$(green "       Instance of Skelpy Node found with:")"
		echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
		echo -e "$(green "       Directory: $Skelpydir")\n"
	else
...
function ntpd {
        ...
        if [ $(systemd-detect-virt) == "lxc" ] || [ $(systemd-detect-virt) == "openvz" ]; then
			...
			sleep 2
                                if ! sudo pgrep -x "ntpd" > /dev/null; then
                                        echo -e "NTP failed to start! It should be installed and running for Skelpy.\n Check /etc/ntp.conf for any issues and correct them first! \n Exiting."
                                        exit 1
                                fi
...
# Logrotate for Skelpy Node logs
function log_rotate {
	if [[ "$(uname)" == "Linux" ]]; then

		if [ ! -f /etc/ .d/Skelpy-logrotate ]; then
			echo -e " Setting up Logrotate for Skelpy node log files."
			sudo bash -c "cat << 'EOF' >> /etc/logrotate.d/Skelpy-logrotate
$HOME/$Skelpydir/logs/skp.log {
        size=50M
        copytruncate
...
function git_upd_check {
	...
	if [ -d "$Skelpydir" ]; then
		...
		if [ "$LOCAL" == "$REMOTE" ]; then
			echo -e "         $(igreen "    SkelpyNode is Up-to-date    \n")"
			UP_TO_DATE=1
...
function purge_pgdb {
	...
	echo -e "    $(ired "   running Skelpy Node processes and will  ")"
...
if [ "$(ls -A $SNAPDIR)" ]; then
	if [[ $(expr `date +%s` - `stat -c %Y $SNAPDIR/current`) -gt 900 ]]; then
		echo -e "$(yellow " Existing Current snapshot is older than 15 minutes")"
        	read -e -r -p "$(yellow "\n Download from Skelpy.ONE? (Y) or use Local (N) ")" -i "Y" YN
			if [[ "$YN" =~ [Yy]$ ]]; then
				echo -e "$(yellow "\n     Downloading latest snapshot from Skelpy.ONE\n")"
				rm $SNAPDIR/current
				wget -nv http://snapshots.skelpy.co/current -O $SNAPDIR/current
				echo -e "$(yellow "\n              Download finished\n")"
			fi
	fi
        ...
        if [[ "$YN" =~ [Yy]$ ]]; then
		echo -e "$(yellow "\n     Downloading current snapshot from Skelpy.ONE\n")"
                wget -nv http://snapshots.skelpy.co/current  -O $SNAPDIR/current
		echo -e "$(yellow "\n              Download finished\n")"
        fi
...
# Install Skelpy Node
function inst_Skelpy {
#	proc_vars
	cd $HOME
        mkdir skelpy-node
        git clone https://github.com/Skelpycoin/skelpy-node.git 2>/dev/null
        cd skelpy-node
...
one(){
	...
	if [ -e $Skelpydir/app.js ]; then
		clear
		asciiart
		echo -e "\n$(green "       ✔ Skelpy Node is already installed!")\n"
		if [ "$node" != "" ] && [ "$node" != "0" ]; then
                	echo -e "$(green "A working instance of Skelpy Node is found with:")"
                	echo -e "$(green "System PID: $node, Forever PID $forever_process")"
        	        echo -e "$(green "and Work Directory: $Skelpydir")\n"
                fi
		pause
		...
		clear
		asciiart
		echo -e "$(yellow "           Installing Skelpy node....")"
		create_db
		inst_Skelpy
		clear
		asciiart
		echo -e "$(green "          ✔ Skelpy node was installed")\n"
		sudo updatedb
		sleep 1
		proc_vars
		log_rotate
...
# Reinstall Skelpy Node
two(){
	clear
	asciiart
	echo -e "$(ired "!!! This option will erase your DB and Skelpy Node installation !!!")\n"
	...
	if [ "$keys" == "Y" ]; then
		proc_vars
        	if [ -e $Skelpydir/app.js ]; then
                	clear
                	asciiart
                	echo -e "\n$(green " ✔ Skelpy Node installation found in $Skelpydir")\n"
                	if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        	echo -e "$(green "A working instance of Skelpy Node is found with:")"
                        	echo -e "$(green "System PID: $node, Forever PID $forever_process")"
				echo -e "$(yellow "           Stopping Skelpy node ...")\n"
				cd $Skelpydir
...
three(){
	...
	if [ "$UP_TO_DATE" -ne 1 ]; then
	        if [ "$node" != "" ] && [ "$node" != "0" ]; then
                	...
                	echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
	                echo -e "$(green "       Directory: $Skelpydir")\n"
			echo -e "\n$(green "             Updating Skelpy Node...")\n"
			update_Skelpy
	                echo -e "$(green "                Restarting...")"
        	        forever restart $forever_process >&- 2>&-
                	echo -e "\n$(green "    ✔ Skelpy Node was successfully restarted")\n"
	                pause
		else
                	echo -e "\n$(red "       ✘ Skelpy Node process is not running")\n"
			...
			echo -e "         $(igreen " Skelpy Node is already Up-to-date \n")"
			sleep 2
	fi
...
four(){
        ...
        if [ "$node" != "" ] && [ "$node" != "0" ]; then
                ...
                echo -e "\n$(green "            Stopping Skelpy Node...")\n"
		cd $Skelpydir
		forever stop $forever_process >&- 2>&-
		echo -e "$(green "             Dropping Skelpy DB...")\n"
                drop_db
		drop_user
		echo -e "$(green "             Creating Skelpy DB...")\n"
		create_db
...
five(){
	...
	if [ "$keys" == "Y" ]; then
        	if [ "$node" != "" ] && [ "$node" != "0" ]; then
			echo -e "\n$(green "       Instance of Skelpy Node found with:")"
			echo -e "$(green "       System PID: $node, Forever PID $forever_process")"
			echo -e "$(green "       Directory: $Skelpydir")\n"
			echo -e "$(green "                Restarting...")"
	                forever restart $forever_process >&- 2>&-
...
start(){
        ...
        if [ -e $Skelpydir/app.js ]; then
                ...
                echo -e "\n$(green "       ✔ Skelpy Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green " A working instance of Skelpy Node was found with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $Skelpydir")\n"
		else
			echo -e "$(green "            Starting Skelpy Node...")\n"
			...
			sleep 1
			proc_vars
			echo -e "\n$(green "       Skelpy Node started with:")"
			echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
			echo -e "$(green "   and Work Directory: $Skelpydir")\n"
                fi
	else
...
status(){
        ...
        if [ -e $Skelpydir/app.js ]; then
                ...
                echo -e "\n$(green "       ✔ Skelpy Node installation found!")\n"
                if [ "$node" != "" ] && [ "$node" != "0" ]; then
                        echo -e "$(green "      Skelpy Node process is working with:")"
                        echo -e "$(green "   System PID: $node, Forever PID $forever_process")"
                        echo -e "$(green "   and Work Directory: $Skelpydir")\n"
                else
                        echo -e "\n$(red "       ✘ No Skelpy Node process is running")\n"
...
killit(){
        ...
        if [ -e $Skelpydir/app.js ]; then
			...
			echo -e "$(green "            Stopping Skelpy Node...")\n"
			cd $testdir
			forever stop $forever_process >&- 2>&-
			cd $parent
			echo -e "$(green "    ✔ Skelpy Node was successfully stopped")\n"
                else
			echo -e "\n$(red "       ✘ No Skelpy Node process is running")\n"
                fi
        else
...
show_menus() {
	...
	echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
	echo "         1. Install Skelpy	2. Reinstall Skelpy			 "
	echo "	 3. Update Skelpy       4. Rebuild Database			 "
	echo "         5. Set/Reset Secret	6. OS Update				 "
	echo "         7. Additional options						 "
	echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
	echo
	echo -e "$(green "                                           S. ")Skelpy Start"
	echo "$(red "                                           K. ")Restart Skelpy"
	echo "$(yellow "                                           E. ")Kill Skelpy"
	echo "                                           L. Node Status"
    echo "                                           P. Node Log"
	echo "                                           0. Exit"
	echo
...
sub_menu() {
	...
	echo "         ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
	echo
	echo "           1. Install Skelpy Cli"
	echo "           2. Install Skelpy Explorer"
...
read_options(){
	local choice
	read -p "        Enter choice [1 - 7,S,K,E,L,P], [0 for exit]: " choice
	case $choice in
		1) one ;;
		2) two ;;
		3) three ;;
...
if [ -e ./.firstrun ] && [ "$fdate" =  "$sdate" ]; then
	...
	if [ ! -e ./.firstrun ] ; then
		...
		echo -e "\n$(ired "    !!!  PLEASE REBOOT YOUR SYSTEM NOW  !!!    ") "
		  ...
		  echo -e "$(ired "    !!!  CHOOSE '1' TO INSTALL Skelpy NODE !!!    ") "
		exit
	fi
fi
